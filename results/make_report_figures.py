...
summary = bna[
    ["file", "weapons", "targets", "bna_time_s", "bna_obj", "bna_status"]
].copy()
summary = summary.rename(
    columns={"bna_time_s": "BnA_time", "bna_obj": "BnA_obj", "bna_status": "BnA_status"}
)
...
andersen_runtime = andersen[["file", "And. time [s]"]].rename(
    columns={"And. time [s]": "Andersen_time"}
)
...
runtime_cmp = runtime_cmp.sort_values(["weapons", "targets", "file"]).reset_index(
    drop=True
)
...
obj_cmp["size_label"] = (
    obj_cmp["weapons"].astype(str) + "x" + obj_cmp["targets"].astype(str)
)
obj_cmp = obj_cmp.sort_values(["mu", "weapons", "targets", "file"]).reset_index(
    drop=True
)
...
legend_items = [
    ...
    Line2D(
        ...
        marker="x",
        linestyle="None",
        label="non-optimal (time/mem/interrupted)",
    ),
]
...
for idx, mu_value in enumerate([1, 2, 3], start=1):
    ...
    ax3d.scatter(
        w_bna, t_bna, part["BnA_obj"], c="#1f77b4", marker="o", s=36, alpha=0.8
    )
    ax3d.scatter(
        w_bna_v2, t_bna_v2, part["BnA_v2_obj"], c="#ff7f0e", marker="s", s=36, alpha=0.8
    )
    ax3d.scatter(
        w_and, t_and, part["Andersen_obj"], c="#2ca02c", marker="^", s=40, alpha=0.8
    )
...
legend_items_3d = [
    ...
    Line2D(
        [0], [0], color="#2ca02c", marker="^", linestyle="None", label="Andersen UB"
    ),
    Line2D(
        [0],
        [0],
...
plt.scatter(
    sizes, bna_v2["bna_status"] == "optimal", label="BnA-v2 optimal", marker="s"
)
